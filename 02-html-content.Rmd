...
This element is used when copyright or related information needs to be added at the bottom of a page. It begins wih `<footer>`, and ends with `</footer>`. Here's an example:
...
There is an element that is used for disclamars or side comments that are spoken quickly in commercials, for example. This is known as the `small` element. Here's an example:
